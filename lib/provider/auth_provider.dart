  ...
  Future<ResponseModel> registration(SignUpModel signUpModel) async {
    ...
    if (apiResponse.response != null &&
      ...
      errorMessage = "email already exist";
        ...
        errorMessage = "email already exist";//errorResponse.errors[0].message;
      }
      print(errorMessage);
  ...
  Future<ResponseModel> login(String email, String password) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
           print("Token is: " );
      Map map = apiResponse.response.data;
      // String map = apiResponse.response.data;
      String token = map["token"];
      // String token = map;
      ...
      String errorMessage;
      if (apiResponse.error is String) {
        errorMessage = "Invalid Login Credential"; //apiResponse.error.toString();
      } else {
        errorMessage = "Invalid Login Credential"; //apiResponse.error;
      }
      print(errorMessage);
      _loginErrorMessage = "Invalid Login Credential ";//errorMessage;
      responseModel = ResponseModel(false, errorMessage);
    }
    _isLoading = false;
    notifyListeners();
